...
public class TaskJ extends ContestTask {
    ...
    public static void main(String[] args) {
        try (BufferedReader r = new BufferedReader(new InputStreamReader(System.in))) {
            int[] input = Arrays.stream(r.readLine().split(" ")).mapToInt(el -> Integer.parseInt(el)).toArray();
            int n = input[0];
            int k = input[1];
            Set<Integer> allParts = IntStream.rangeClosed(1, 200).mapToObj(el -> el).collect(Collectors.toSet());
            Set<Integer>[] arr = new Set[n];
            int period = 0;
            Map<Integer, List<Integer>> partsOnDevices = new HashMap<>();
            Map<Integer, List<Integer>> devices = new HashMap<>();
            Set<Integer> transferCompleted = new HashSet<>();
            while (transferCompleted.size() != n) {
                period++;
                for (Map.Entry<Integer, List<Integer>> device : devices.entrySet()) {
                    Integer deviceIndex = device.getKey();
                    List<Integer> parts = device.getValue();
                    if (parts.size() == k) {
                        transferCompleted.add(deviceIndex);
                        continue;
                    }
                    Optional<Map.Entry<Integer, List<Integer>>> any = partsOnDevices.entrySet().stream()
                            .filter(entry -> allParts.stream().filter(el -> !parts.contains(el)).collect(Collectors.toSet()).contains(entry.getKey()))
                            .sorted(Comparator.comparingInt(o -> o.getValue().size()))
                            .findAny();
                    if (any.isPresent()) {
                        Integer part = any.get().getKey();
                        List<Integer> deviceWithPart = partsOnDevices.getOrDefault(part, new ArrayList<>());
                        if (deviceWithPart.size() == 1) {
                            List<Integer> deviceDetails = devices.getOrDefault(deviceIndex, new ArrayList<>());
                            deviceDetails.add(deviceWithPart.get(0));
                            devices.put(deviceIndex, deviceDetails);
                        } else {
                            Optional<Integer> first = deviceWithPart.stream().sorted(new Comparator<Integer>() {
                                @Override
                                public int compare(Integer o1, Integer o2) {
                                    int size1 = devices.getOrDefault(o1, new ArrayList<>()).size();
                                    int size2 = devices.getOrDefault(o2, new ArrayList<>()).size();
                                    if (size1 == size2) {
                                        return o1 - o2;
                                    } else {
                                        return size1 - size2;
