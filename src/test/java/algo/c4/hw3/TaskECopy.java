...
public class TaskECopy extends ContestTask {
    ...
    public static void main(String[] args) {
        try (BufferedReader r = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            Map<Integer, BigDecimal> results = new LinkedHashMap<>();
            for (int fromIndex = 1; fromIndex <= cityAmount; fromIndex++) {
                ...
                q.add(new Chunk(from, from.originSpeed, from.prepareHours, new ArrayList<>(List.of(from)), new ArrayList<>(List.of(from))));
                ...
                while (!q.isEmpty()) {
                    ...
                    for (Road road : roads) {
                        ...
                        if (finishCity.totalTime > roadTotalTime || finishCity.totalSpeed < startSpeed) {
                            finishCity.totalTime = roadTotalTime;
                            finishCity.totalSpeed = startSpeed;
                            ...
                            q.add(new Chunk(finishCity, finishCity.originSpeed, roadTotalTime + finishCity.prepareHours, updatedPath, previousFullPath));
                        }
                    }
                ...
                results.put(fromIndex, BigDecimal.valueOf(destination.totalTime).setScale(10, RoundingMode.HALF_UP));
            }
            maxPath.add(destination);
            System.out.println(BigDecimal.valueOf(maxTotalTime).setScale(10, RoundingMode.HALF_UP));
            System.out.println("0.0000000000 " + results.entrySet().stream().map(el -> String.valueOf(el.getValue())).collect(Collectors.joining(" ")));
...
//        String expected = getFileContent(answerFilePath);
    ...
    static class City {
        ...
        private final int originSpeed;
        private int totalSpeed;
        private List<City> path = new ArrayList<>();
        private List<City> fullPath = new ArrayList<>();
        private final List<Road> roads = new ArrayList<>();

        public City(int value, int prepareHours, int originSpeed) {
            this.value = value;
