...
/*
https://contest.yandex.ru/contest/53029/problems/F/
Миша и математика
...
public class TaskF extends ContestTask {
    ...
    public static void main(String[] args) {
        try (BufferedReader r = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            int[] nums = Arrays.stream(r.readLine().split(" ")).filter(el -> el != null && !el.isEmpty()).mapToInt(el -> Integer.parseInt(el)).toArray();
            Map<Integer, Queue<Point>> map = new HashMap<>();
            Queue<Point> q = new ArrayDeque<>();
            q.add(new Point((nums[0] & 1) == 0 ? true : false, ""));
            int stage = 0;
            map.put(stage, q);
